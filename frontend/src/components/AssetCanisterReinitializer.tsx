...
export default function AssetCanisterReinitializer() {
  ...
  const runPostChecks = async () => {
    ...
    try {
      ...
      setPostCheckStatus('degraded');
      toast.error('Проверка после реинициализации не удалась: ' + error.message);
      return false;
    }
  };
  ...
  const reinitializeAssetCanister = async () => {
    ...
    if (!preChecksPassed) {
      toast.error('Предварительные проверки не пройдены. Реинициализация отменена.');
      return;
    }
    ...
    try {
      console.log('═══════════════════════════════════════════════════════════════════════════');
      console.log('🔄 [Asset Canister Reinitialization] Starting targeted reinitialization');
      console.log('═══════════════════════════════════════════════════════════════════════════');
      ...
      updateStep('Проверка подключения к Asset Canister', 'success', 'Actor доступен');
      ...
      try {
        ...
        console.log('[Step 2/4] ✓ initializeAccessControl() completed successfully');
        updateStep('Вызов initializeAccessControl()', 'success', 'Контроль доступа реинициализирован');
        ...
        if (error.message && error.message.includes('already initialized')) {
          console.log('[Step 2/4] ⚠ Access control already initialized (this is OK)');
          updateStep('Вызов initializeAccessControl()', 'success', 'Уже инициализирован');
      ...
      try {
        ...
        console.error('[Step 3/4] ✗ Failed to verify access control:', error.message);
      ...
      try {
        ...
        updateStep('Проверка работоспособности Land Canister', 'success', 'Полностью работоспособен');
      } catch (error: any) {
        console.error('[Step 4/4] ✗ Land Canister check failed:', error.message);
        updateStep('Проверка работоспособности Land Canister', 'failed', error.message);
        throw error;
      }

      console.log('═══════════════════════════════════════════════════════════════════════════');
      console.log('✅ [Asset Canister Reinitialization] Completed successfully');
      console.log('═══════════════════════════════════════════════════════════════════════════');
  ...
  const getStatusBadge = (status: typeof preCheckStatus | typeof postCheckStatus) => {
    switch (status) {
      case 'idle':
        return null;
      case 'checking':
        return <Badge className="bg-cyan-500/20 text-cyan-400 border-cyan-500/50">Проверка...</Badge>;
      case 'ready':
        return <Badge className="bg-green-500/20 text-green-400 border-green-500/50">Готов</Badge>;
      case 'healthy':
        return <Badge className="bg-green-500/20 text-green-400 border-green-500/50">Работает</Badge>;
      case 'degraded':
        return <Badge className="bg-yellow-500/20 text-yellow-400 border-yellow-500/50">Частично</Badge>;
      case 'failed':
        return <Badge className="bg-red-500/20 text-red-400 border-red-500/50">Ошибка</Badge>;
    }
  };
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="text-xs text-gray-300 space-y-1">
                  <p>Реинициализация Asset Canister доступна только авторизованному администратору.</p>
                  <p className="mt-2 text-gray-400">
                    Текущий Principal: <code className="text-xs bg-black/30 px-1 py-0.5 rounded">{identity?.getPrincipal().toString()}</code>
                  </p>
                  <p className="mt-2 text-gray-400">
                    Требуется Principal: <code className="text-xs bg-black/30 px-1 py-0.5 rounded">whd5e-pbxhk-pp65k-hxqqx-edtrx-5b7xd-itunf-pz5f5-bzjut-dxkhy-4ae</code>
                  </p>
                </div>
              </div>
            </div>
      ...
      <CardContent className="space-y-6">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <ul className="text-xs text-gray-300 space-y-1">
                <li>• Будет реинициализирован только <strong>Asset Canister</strong></li>
                <li>• <strong>Land Canister</strong> останется нетронутым и полностью работоспособным</li>
                <li>• Процедура восстановит контроль доступа Asset Canister</li>
                <li>• После завершения будет выполнена проверка работоспособности</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        {steps.length > 0 && (
          <div className="space-y-2">
            <div className="text-sm font-medium text-gray-300 mb-3">Шаги реинициализации:</div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="text-xs text-gray-300">
                  Все проверки работоспособности пройдены успешно. Asset Canister полностью восстановлен.
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 rounded-lg bg-yellow-500/10 border border-yellow-500/30">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="text-xs text-gray-300">
                  Некоторые проверки не прошли. Проверьте логи диагностики для деталей.
                </div>
              </div>
            </div>
          </div>
