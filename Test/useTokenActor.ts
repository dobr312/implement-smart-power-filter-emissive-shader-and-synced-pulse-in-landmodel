...
async function selectOptimalGateway(
  ...
  for (const gateway of gateways) {
    try {
      ...
      console.warn(
        `[CyberToken] Gateway ${gateway} unavailable, trying next...`,
      );
    }
  }
  ...
  useEffect(() => {
    ...
    const initActorWithRetry = async () => {
      ...
      for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
        try {
          console.log(
            `[CyberToken Actor] Initialization attempt ${attempt + 1}/${MAX_RETRIES} with 120s timeout`,
          );
          ...
          console.log("[CyberToken Actor] Config:", {
          ...
          const timeoutPromise = new Promise<never>((_, reject) => {
            setTimeout(
              () => reject(new Error("Token actor timeout (120s)")),
              ACTOR_TIMEOUT,
            );
          });
          ...
          console.error(
            `[CyberToken Actor] Attempt ${attempt + 1} failed:`,
            errorMessage,
          );
