...
): Promise<{ healthy: boolean; latency: number | null }> {
  try {
...
async function selectOptimalGateway(
  ...
  const sortedGateways = healthChecks
    ...
    .sort(
      (a, b) =>
        (a.latency || Number.POSITIVE_INFINITY) -
        (b.latency || Number.POSITIVE_INFINITY),
    );

  if (sortedGateways.length > 0) {
    const selected = sortedGateways[0];
    console.log(
      `[AssetCanister] ✓ Gateway selected: ${selected.gateway} (latency: ${selected.latency}ms)`,
    );
    return selected.gateway;
  }

  console.warn(
    "[AssetCanister] All gateways failed health check, using primary (ic0.app)",
  );
  return gateways[0];
}
...
export function useAssetActor() {
  ...
  const [envValidation, setEnvValidation] =
    useState<EnvValidationResult | null>(null);
  ...
  useEffect(() => {
    ...
    if (currentIdentityRef.current !== currentIdentityStr) {
      console.log(
        "[AssetCanister] Identity changed, forcing full reconnection...",
      );
    ...
    if (!validation.isValid) {
      console.error(
        "[AssetCanister Actor] Environment validation failed:",
        validation.errors,
      );
    ...
    const initActorWithRetry = async () => {
      ...
      for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
        try {
          console.log(
            `[AssetCanister Actor] Initialization attempt ${attempt + 1}/${MAX_RETRIES} with 120s timeout`,
          );
          ...
          const { assetCanisterId, network, gateways } =
            currentValidation.config!;

          const host =
            network === "local"
              ? LOCAL_HOST
              : await selectOptimalGateway(gateways, assetCanisterId);
          ...
          const timeoutPromise = new Promise<never>((_, reject) => {
            setTimeout(
              () => reject(new Error("Asset actor timeout (120s)")),
              ACTOR_TIMEOUT,
            );
          });
          ...
          const actorPromise = (async () => {
            ...
            if (network === "local") {
              try {
                ...
                console.warn(
                  "[AssetCanister Actor] Root key fetch failed:",
                  err,
                );
              }
            }
          ...
          try {
            console.log(
              "[AssetCanister Actor] Testing connectivity with listAssets...",
            );

            const healthCheckPromise = (async () => {
              // Use the new lightweight listAssets endpoint
              const _quickAssets = await newActor.listAssets();
              console.log(
                "[AssetCanister Actor] ✓ Quick health check passed (listAssets)",
              );
            })();
            ...
            console.warn(
              "[AssetCanister Actor] Connectivity test failed:",
              testError,
            );
          ...
          console.log(
            "[AssetCanister Actor] ✅ Actor initialized successfully",
          );
          console.log(
            "═══════════════════════════════════════════════════════════════════════════",
          );
          ...
          console.error(
            `[AssetCanister Actor] Attempt ${attempt + 1} failed:`,
            errorMessage,
          );
