...
export const MAINNET_CONFIG = {
  ...
  canisters: {
    governance:
      process.env.GOVERNANCE_CANISTER_ID || "ta3s3-myaaa-aaaau-afx4a-cai",
    ...
    marketplace:
      process.env.MARKETPLACE_CANISTER_ID || "be2us-64aaa-aaaaa-qaabq-cai",
...
export function initializeMainnetConfig(canisterIds: {
  ...
  MAINNET_CONFIG.plugWallet.whitelist = Object.values(
    MAINNET_CONFIG.canisters,
  ).filter(Boolean);
...
export function validateMainnetConfig(): boolean {
  ...
  for (const canister of requiredCanisters) {
    if (
      !MAINNET_CONFIG.canisters[
        canister as keyof typeof MAINNET_CONFIG.canisters
      ]
    ) {
...
export function getMainnetModelUrl(tier: number): string {
  ...
  const filename =
    MAINNET_CONFIG.modelFilenames[
      `tier${tier}` as keyof typeof MAINNET_CONFIG.modelFilenames
    ];
